<div class="panel panel-default panel-color">
            ...
            <form id="fm_data" onsubmit="return false">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="form-group" id="transfer1">
							<label>Transfer Bank</label>
							 <select class="form-control" id="kdbank" name="kdbank">
								<option value="-">[-PILIH-]</option>
							</select>
						</div>
						<div class="form-group" id="transfer2">
                            <label>Nominal Transfer</label>
                            <div class="input-group">
                                <div class="input-group-addon">Rp</div>
                                <input type="text" id="jm_transfer" name="jm_transfer" class="form-control number_format" autocomplete="off">
                            </div>
                        </div>
    ...
    <div class="panel-heading">
		...
		<button class="btn btn-success btn-small" onclick="edit_margin()"> Edit Margin</button>
...
<!-- edit margin -->
<div class="modal fade" id="myModalMarginss2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title" id="myModalLabel">Update Margin</h4>
            </div>
            <div class="modal-body">
                <form id="form_marginss2">
                    <div class="form-group">
                        <label>Margin (%)</label>
                        <input type="text" name="margin" id="margin" class="form-control" autocomplete="off" style="text-transform: uppercase;">
                        <input type="hidden" name="no_simpan" id="no_simpan">
                    </div>
                </form>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-primary" onclick="simpan_margin_ss2()">Simpan</button>
                <button type="button" class="btn btn-default" data-dismiss="modal">Batal</button>
            </div>
        </div>
    </div>
    ...
    if ($.fn.DataTable.isDataTable("#tabel_sukarela2")) {
        ...
        if (data_noss2) {
			if (data_noss2.tgl_debet != null) {
				...
				$("#myModalMarginss2").modal('show');
				set_form("form_marginss2", data_noss2);
			}
        } else {
            alert("Pilih data di tabel");
        }
...
function simpan_margin_ss2() {
    data_form_noss2 = $("#form_marginss2").serialize();

    validasi = $("#form_marginss2").valid();
    ...
    if (validasi) {
        ...
        if (prompt) {
            $.ajax({
                url: situs + "simpanan/update_margin_ss2",
                data: data_form_noss2,
                dataType: "JSON",
                type: "POST",
                beforeSend: function() {
                ...
                success: function(res) {
                   if(res == 1){
					   $("#myModalMarginss2").modal('hide');
					   alert("Data Berhasil Diupdate");
					    get_transaksi_simpanan();
				   }
				   else{
					    alert("Data Gagal Diupdate,Simpanan ini Sudah Berjalan");
				   }
                }
            });
        }
...
$("#kdbank").select2({
	width: '100%',
	ajax: {
		url: situs + "master/select_kasbank",
		dataType: 'json',
		delay: 500
	}
});
