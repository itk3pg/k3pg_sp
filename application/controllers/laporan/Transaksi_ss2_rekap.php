...
class Transaksi_ss2_by_nak extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['judul_menu'] = "Laporan Transaksi SS2 Urut sesuai NAK";
        $data['bulan']      = $bulan;

        $this->template->view("laporan/transaksi_ss2_by_nak", $data);
    }

    private function query_ss2($tgl_awal, $tgl_akhir)
    {
        $qdata_ss2 = "SELECT no_simpan, tgl_simpan, waktu_simpan, no_ss2, unit_adm, no_ang, no_peg, nm_ang, kd_prsh, nm_prsh, kd_dep, nm_dep, kd_bagian, nm_bagian, kd_jns_simpanan, nm_jns_simpanan, kd_jns_transaksi, nm_jns_transaksi, jml_simpanan, tempo_bln, margin, jml_margin, jml_margin_bln, tgl_jt, is_debet, tgl_debet, jml_debet, jml_denda, jenis_bayar_denda, is_dipercepat, is_diperpanjang, no_bukti_baru, ket, user_input, tgl_insert, user_edit, tgl_update
            FROM (
                SELECT * FROM t_simpanan_sukarela2
                WHERE is_dipercepat = '0'
                    and (tgl_simpan >= '" . $tgl_awal . "' or '" . $tgl_akhir . "' <= tgl_jt)
                order BY no_ang, no_ss2, tgl_simpan DESC
            ) a
            GROUP BY no_ss2
            ORDER BY no_ang, LENGTH(no_ss2), no_ss2";

        return $this->db->query($qdata_ss2);
    }
    ...
    public function tampilkan()
    {
        ...
        if ($data_req) {
            ...
            $data_ss2 = $this->query_ss2($tgl_awal, $tgl_akhir);
    ...
    public function cetak()
    {
        ...
        $judul_file   = "cetak_ss2_urut_nak_" . date("Y-m-d_H-i-s") . ".pdf";
        ...
        $pdf->Cell(0, 0, "Laporan Simpanan Sukarela 2 Urut NAK", 0, 0, "C");
        $pdf->SetFontSize('8');
