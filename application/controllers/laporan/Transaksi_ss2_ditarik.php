...
class Transaksi_ss2_ditarik extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['judul_menu'] = "Laporan Transaksi SS2 Ditarik";
        $data['bulan']      = $bulan;

        $this->template->view("laporan/transaksi_ss2_ditarik", $data);
    }
        ...
        if ($data_req) {
            $laporan = "<table class=\"table table-bordered table-condensed table-striped\" style=\"white-space: nowrap;\">
                <thead>
                    ...
                    <tr>
                        <th style=\"text-align: center;vertical-align: middle;\">Ditarik</th>
            ...
            $data_ss2 = $this->db->where("(tgl_debet between '" . $tgl_awal . "' and '" . $tgl_akhir . "')")
                ->where("is_dipercepat", "1")
                ->where("is_ubah_tempo", "0")
                ->order_by("tgl_debet, no_ang")
            ...
            foreach ($data_ss2->result_array() as $key => $value) {
                ...
                $laporan .= "
                    <tr>
                        ...
                        <td style=\"text-align: right;\">" . balik_tanggal($value['tgl_debet']) . "</td>
    ...
    public function cetak()
    {
        ...
        $judul_file   = "cetak_ss2_ditarik_" . date("Y-m-d_H-i-s") . ".pdf";
        ...
        $pdf->Cell(0, 0, "Laporan Simpanan Sukarela 2 Ditarik", 0, 0, "C");
        $pdf->SetFontSize('8');
        ...
        $pdf->Cell($koleng1, 10, "");
        $pdf->Cell($koleng[6], 5, "Ditarik", 1, '', "C");
        ...
        foreach ($data_ss2->result_array() as $key => $value) {
            ...
            $pdf->Cell($koleng[6], 0, balik_tanggal($value['tgl_debet']), 0, '', "C");
            $pdf->Cell($koleng[7], 0, balik_tanggal($value['tgl_jt']), 0, '', "C");
